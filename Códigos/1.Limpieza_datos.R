...
# Histograma: Análisis de precios de propiedades --------------------------------

# Frecuencia -------------------------------------------------------------------

# Forma 1: Histograma básico de la variable `price` (en millones)
# Gráfico de la distribución del precio, con valores ajustados a millones y un diseño básico.
p <- ggplot(train, aes(x = price / 1000000)) +
  geom_histogram(fill = "darkblue", alpha = 0.4) + # Barras con color azul oscuro y transparencia
...
# Forma 2: Histograma con escala logarítmica
# Gráfico que transforma los precios a escala logarítmica para mejorar la visualización de datos sesgados.
p <- ggplot(train, aes(x = Log_precio)) +
  geom_histogram(fill = "darkblue", alpha = 0.4) + # Barras con color azul oscuro y transparencia
  labs(x = "Valor de venta (log-scale)", y = "Cantidad") + # Etiquetas de los ejes
  scale_x_log10(labels = scales::dollar) + # Escala logarítmica con formato monetario
  theme_bw() # Tema básico en blanco y negro
ggplotly(p) # Hacer el gráfico interactivo con ggplotly

# Densidad ---------------------------------------------------------------------

# Histograma de densidad del precio (en millones)
# Gráfico básico en R para visualizar la densidad de los precios.
hist((train$price / 1000000), probability = TRUE, col = '#A6CFE2', border = "grey30", breaks = 25, 
     xlab = "Precio (Millones)", main = "Distribución del Precio (en millones)") # Histograma con densidad
abline(v = mean((train$price / 1000000)), col = 'red', lwd = 3, lty = 2) # Línea vertical para la media
lines(density((train$price / 1000000)), col = '#00008B', lwd = 3) # Línea de densidad
legend("topright", legend = c("Media", "Densidad"), col = c("red", "#00008B"), lty = c(2, 1), lwd = 3, bty = "n") # Leyenda

# Frecuencias por tipo de propiedad --------------------------------------------

# Calcular medias del precio por tipo de propiedad
# Las medias se calculan en millones para facilitar la interpretación.
...
# Gráfico de histogramas por tipo de propiedad
# Este gráfico muestra la distribución del precio segmentado por tipo de propiedad.
tipo <- ggplot(train, aes(x = (price / 1000000), fill = property_type)) +
  ...
  geom_vline(aes(xintercept = media_apto), color = "steelblue", linetype = "dashed", size = 1) + # Línea media apartamento
  geom_vline(aes(xintercept = media_casa), color = "grey50", linetype = "dashed", size = 1) + # Línea media casa
  labs(
    title = "Distribución de Precios de Viviendas", # Título del gráfico
    x = "Precio de Viviendas (Millones)", # Etiqueta del eje X
    y = "Frecuencia" # Etiqueta del eje Y
  ) +
  scale_fill_manual(values = c("Apartamento" = "steelblue", "Casa" = "grey60"), guide = "none") + # Colores por tipo
  scale_x_continuous(
...
#--------------------#
#  Crear variables nuevas imputadas por barrio
#--------------------#

##. Imputación por moda de las variables `rooms` y `bathrooms`  agrupadas por tipo de propiedad y barrio

# Identifica el tipo de cada variable antes de la imputación
sapply(train[, c("rooms", "bathrooms")], class)
...
# Imputación por media de `surface_total` y `surface_covered`
train <- train %>%
  group_by(property_type, barrio) %>%
  mutate(
...
# Imputación por mediana de `surface_total` y `surface_covered`
train <- train %>%
  group_by(property_type, barrio) %>%
  mutate(
...
# Base `test`:
# Aplica las mismas transformaciones e imputaciones para `test`
...
# Imputación por media para `test`
test <- test %>%
  group_by(property_type, barrio) %>%
  mutate(
    surface_total_imp_mean3 = ifelse(is.na(surface_total), mean(surface_total, na.rm = TRUE), surface_total),
...
# Imputación por mediana para `test`
test <- test %>%
  group_by(property_type, barrio) %>%
  mutate(
    surface_total_median3 = ifelse(is.na(surface_total), median(surface_total, na.rm = TRUE), surface_total),
